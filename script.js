...
async function playGame(_option, random_type){
    if (random_type == "pseudo") {
        amountInEth = document.getElementById("amountInEth").value;
        amountInWei = ethers.utils.parseEther(amountInEth.toString())
        console.log(amountInWei);
        resultOfCoinFlip = await contract.play_bot_pseudo_bnb(_option, {value: amountInWei});

    } else {
        amountInEth = document.getElementById("amountInEth2").value;
        amountInWei = ethers.utils.parseEther(amountInEth.toString())
        console.log(amountInWei);
...
async function handleEvent(){
    ...
    let queryResult =  await contract.queryFilter('PlayGame', blockNum - 3000, blockNum);
